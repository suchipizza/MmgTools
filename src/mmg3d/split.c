 ...
 * \param met pointer toward the metric structure.
 * \param k index of element to split.
 * \param ifac local index of face to split.
 * \param o coordinates of inserted point.
 * \param cb barycentric coordinates of inserted point.
 * \param ip pointer toward the global index of the new point.
 * \return -1 if fail, the local index of the new point in the element otherwise.
 *
 * Split a tetra in 3 tetras by introducing the point of coordinates \a o
 * and barycentric coordinates \a cb in the face \a ifac;
 * Update adjacency relations and return local index of the new point in \a k.
 *
 */
int _MMG5_split3cb(MMG5_pMesh mesh, MMG5_pSol met, int k, int ifac, double o[3],
  ...
  if ( met->m )
    _MMG5_interp4bar(mesh,met,k,(*ip),cb);
  ...
  /* Quality update */
  pt[0]->qual=_MMG5_orcal(mesh,met,newtet2[0]);
  pt[1]->qual=_MMG5_orcal(mesh,met,newtet2[1]);
  pt[2]->qual=_MMG5_orcal(mesh,met,newtet2[2]);
 ...
 *
 * Split a tetra in 4 tetras by introducing a point \a o whose barycentric
 * coordinates \a cb are known.  Update adjacency relations.
 *
 */
int _MMG5_split4cb(MMG5_pMesh mesh, MMG5_pSol met, int k, double o[3],
                   double cb[4], int *ip ) {
  MMG5_pTetra        pt[4];
  MMG5_xTetra        xt[4];
  MMG5_pxTetra       pxt0;
